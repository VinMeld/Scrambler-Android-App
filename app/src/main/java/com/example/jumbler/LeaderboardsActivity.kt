        ...
        if ((this.application as Jumbler).isDeviceOnline()) {
            ...
            scopeLeaderboard.launch(Dispatchers.Default) {
                    ...
                    try {
                        user.document(userID).get()
                            ...
                            .addOnFailureListener { exception ->
                                runOnUiThread {
                                    progressBar.visibility = View.INVISIBLE
                                    emptyLeaderboardText.text =
                                        getString(R.string.leaderboard_error)
                                    emptyLeaderboardText.visibility = View.VISIBLE
                                }
                                        ...
                                        for (documents in userSnapshot) {
                                            (documents.get("scores") as MutableList<Int>?)?.forEachIndexed { _, score ->
                                                if (score != 0) {
                                                    val newEntry =
                                                        UserObject(
                                                            documents.get("username").toString(),
                                                            score
                                                        )
                                                    scores.add(newEntry)
                                                }
                ...
                runOnUiThread {
                    ...
                    leaderboardTab.addOnTabSelectedListener(object :
                        TabLayout.OnTabSelectedListener {
                        override fun onTabSelected(tab: TabLayout.Tab?) {
                            if (tab?.position == 0) {
                                leaderboardRecycler.visibility = View.VISIBLE
                                if (leaderboardIsEmpty) emptyLeaderboardText.visibility =
                                    View.VISIBLE
                                globalLeaderboardRecycler.visibility = View.INVISIBLE
                            } else {
                                leaderboardRecycler.visibility = View.INVISIBLE
            ...
            runOnUiThread {
                progressBar.visibility = View.INVISIBLE
                emptyLeaderboardText.text = getString(R.string.leaderboard_offline)
                emptyLeaderboardText.visibility = View.VISIBLE
            }
        }
    }
